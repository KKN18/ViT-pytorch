...
class Convolution(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        cnn_layer = hidden_states

        # [51, 197, 768]
        k_shape = cnn_layer.shape
       
        global PATCH_X # 14
        global PATCH_Y # 14 
  
        # remove cls token
        cnn_cls, cnn_layer = torch.split(k_layer, [1, k_shape[1]-1], dim=1)

        #shape without cls [51, 196, 768] = [B, HxW, C]

        # CNN Code
        # reshape from [B, HxW, C] to [B, H, W, C]
        cnn_layer1 = cnn_layer.reshape(k_shape[0], PATCH_X, PATCH_Y, k_shape[2])

        # change to CNN input dimension [B, C, H, W]
        cnn_layer1 = cnn_layer1.permute(0, 3, 1, 2).contiguous()

        # CNN Forward
        conv_layer = self.seq_conv2d(cnn_layer1)

        # restore original dimension [B, H/2, W/2, C]
        conv_layer = conv_key_layer.permute(0, 2, 3, 1).contiguous()

        # FC Code
        # permute to [B, C, HxW]
        cnn_layer2 = cnn_layer.permute(0, 2, 1).contiguous()

        # Linear Forward [B, C, H/2xW/2]
        fc_cnn_layer = self.linear(cnn_layer2)
        
        # reshape to [B, C, H/2, W/2]
        fc_cnn_layer = fc_cnn_layer.reshape(k_shape[0], k_shape[2], PATCH_X//2, PATCH_Y//2)

        # restore original dimension [B, H/2, W/2, C]
        fc_cnn_layer = fc_cnn_layer.permute(0, 2, 3, 1).contiguous()

        # Add CNN Code and FC Code
        total_cnn_layer = conv_layer + fc_cnn_layer

        # restore initial shape without cls [51, 49, 768]
        after_conv_shape = (k_shape[0], self.n_out ** 2, k_shape[2])
        total_cnn_layer = total_cnn_layer.reshape(after_conv_shape)

        # cat cls token
        cat_cnn_layer = torch.cat((cnn_cls, total_key_layer), dim=1)

        cnn_output = self.softmax(cat_cnn_layer)

        assert(self.vis = False)

        return cnn_output, None
...
class CNN_Block(nn.Module):
    def __init__(self, config, vis):
        ...
        self.hidden_size = config.hidden_size
        self.attention_norm = LayerNorm(config.hidden_size, eps=1e-6)
        self.cnn = Convolution(config, vis)

    def forward(self, x):
        h = x
        x = self.attention_norm(x)
        x, weights = self.cnn(x)
        x = x + h
        h = x
        x = self.ffn_norm(x)
        x = self.ffn(x)
